...
if single_knet
    [idx, vals, Pnts_Scores]= knet_cluster(Dists, Neighbs, K, kstep, ...
        resolve, c, maxiters);
elseif knet_serial_knet
    [tidx, vals]= knet_cluster(Dists, Neighbs, K(1), 1, ...
        1, -1, maxiters);
    meds=unique(tidx);nDists=Dists(meds,meds);
    if nlin > 0
        nDists = nlinmap(nDists, nlin);
    end
    [idx, vals]= knet_cluster(nDists, Neighbs, K(2), kstep, ...
        resolve, c, maxiters);
    if nlin == 0
        fmeds=unique(idx);
...
function idx = partial_knet_iters(data, pmeds, Tidx, ...
    k, kstep, resolve, c, maxiters, metric)
ndata=data(pmeds, :);
